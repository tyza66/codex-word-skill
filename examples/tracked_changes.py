...
codex-word-skill documents for editors: wrap the replacement at the OOXML
...
def add_tracked_replacement(docx_path: str, output_path: str, old: str, new: str, author: str = "Codex") -> None:
    ...
    for p in root.iter(f"{W}p"):
        for r in p.findall(f"{W}r"):
            ...
            if t is not None and t.text and old in t.text:
                ...
                ins = _make_ins_run(t.text, author, "2026-09-16T00:00:00Z", r)
                p.replace(r, ins)
                replaced = True
                break
        if replaced:
